...
**Публична редакция:** 27.08.2026 г.  
**Обхват:** текущо публикувания ArsExam **3.2.0 Stable**. Test каналът временно е синхронизиран със същата версия и няма активен prerelease.
...
В зависимост от използваните функции ArsExam 3.2.0 може да съхранява на устройството:
...
ArsExam 3.2.0 използва application-level encryption за основната SQLite база и отделни защитени локални registry/security файлове.
...
Функцията **„Забравена парола?“** работи локално. ArsExam 3.2.0 не използва Supabase, recovery e-mail, Request ID, support code или друг online control plane за password recovery.
...
Crash/error диагностиката е **opt-in** и е изключена по подразбиране. ArsExam 3.2.0 не събира отделна usage/behavior analytics телеметрия за това кои екрани, банки или функции използвате.
...
При валидно съгласие ArsExam може да пази ограничена локална минимизирана diagnostic history. Изключването на диагностиката спира бъдещото дистанционно изпращане и прилага локалната cleanup логика на приложението.

Remote delivery е възможно само ако конкретният build е конфигуриран с валиден **Sentry EU/DE** ingest канал и потребителят е дал изрично съгласие. ArsExam ограничава production ingest до одобрения Sentry EU/DE host pattern; не приема произволен външен endpoint за този канал.
...
ArsExam 3.2.0 **не използва Supabase за password recovery**.
...
Локалната diagnostic history е ограничена от приложната retention логика и се управлява според diagnostics consent/settings.
...
Тази публична редакция описва текущо публикувания ArsExam **3.2.0 Stable** към 27.08.2026 г. Test каналът в момента е синхронизиран със същата версия.

При нов публичен release документът трябва да бъде прегледан и синхронизиран с действителното поведение на конкретната версия. Release-specific документът, доставен с конкретен Setup, остава част от release evidence за тази версия.
